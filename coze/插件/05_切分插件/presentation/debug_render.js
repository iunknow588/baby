...
async function renderGridDebugImage(imageInput, debugOutputPath, debugData) {
  ...
  if (debugData.guideAxisModes) {
    addLabel(16, 112, `轴模式: X=${debugData.guideAxisModes.x || '-'} / Y=${debugData.guideAxisModes.y || '-'}`, '#7c2d12');
  }
  if (debugData.guidePatternFallback) {
    addLabel(16, 144, `模式回退: X=${debugData.guidePatternFallback.x ? '是' : '否'} / Y=${debugData.guidePatternFallback.y ? '是' : '否'}`, '#0f766e');
  }
  if (debugData.guideAnchorPreference) {
    addLabel(16, 176, `锚点偏置: X=${debugData.guideAnchorPreference.x || '-'} / Y=${debugData.guideAnchorPreference.y || '-'}`, '#1d4ed8');
  }
  addLabel(16, 208, '图例: 橙色=候选线', '#92400e');
  addLabel(16, 240, isBoundaryGuideMode ? '引导竖横线=深青/紫' : '选中竖横线=蓝/红', isBoundaryGuideMode ? '#0f766e' : '#1d4ed8');
  addLabel(16, 272, '绿色虚线=外框锚线', '#166534');
  addLabel(16, 304, '青/粉=轮廓或纠偏前', '#475569');
