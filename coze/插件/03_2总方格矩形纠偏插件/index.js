...
class GridRectAdjustPlugin {
  ...
  async execute(params) {
    ...
    const isDirectSegmentationInput = typeof bounds.source === 'string' && bounds.source.includes('03_0_6_单格切分输入直通');
    ...
    if (!shouldAdjust) {
      const payload = {
        ...
        processName: '03_2_总方格矩形纠偏',
        sourceStep: '03_1_总方格候选矩形',
        bounds,
        adjusted: false,
        diagnostics,
        adjustment: {
  ...
  async renderAdjustmentAnnotation(inputPath, outputPath, bounds, adjusted) {
    ...
    const svg = `
      <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="34" y="50" font-size="24" fill="#ffffff">03_2 总方格矩形纠偏</text>
